...
export default function PatientDashboard() {
  ...
  const PAYMENT_CONTRACT_ADDRESS = "0x66Bd396353701d97a7C21A23f57044761133dcD5";
  ...
  const loadBlockchainRecords = async (patientIdentifier) => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/medical-records/on-chain/${patientIdentifier}`,
        { headers },
      );
      if (res.data.success) {
      ...
      setBlockchainError(
        `Không thể đồng bộ thời gian thực với Blockchain Sepolia: ${err.response?.data?.message || err.message}`,
      );
    } finally {
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/doctors/hospitals`, {
          headers,
        });
        if (res.data?.success) {
        ...
        console.error(
          "Lỗi tải danh sách bệnh viện từ API (Có thể bỏ qua nếu dùng danh sách cứng):",
          err.message,
        );
      }
    };
    ...
    const loadMedicalHistory = async () => {
      try {
        const res = await fetch(`${BASE_URL}/visits/my?patientId=${userId}`, {
          headers,
        });
        const data = await res.json();
  ...
  const loadInvoices = async () => {
    ...
    try {
      ...
      setInvoiceError("Lỗi kết nối máy chủ hóa đơn!");
    } finally {
  ...
  const loadAccessRequests = async () => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/access/requests/my?patientId=${userId}`,
        { headers },
      );
  ...
  const handlePayWithMetaMask = async (invoice) => {
    ...
    if (!window.ethereum) {
      setInvoiceError(
        "Hệ thống không tìm thấy ví MetaMask. Vui lòng cài đặt tiện ích này!",
      );
      return;
    }
    ...
    try {
      ...
      if (
        invoice.patientWallet &&
        walletAddress.toLowerCase() !== invoice.patientWallet.toLowerCase()
      ) {
        setInvoiceError(
          `Hóa đơn này được tạo riêng cho ví: ...${invoice.patientWallet.slice(-6)}. Ví hiện tại của bạn là: ...${walletAddress.slice(-6)}`,
        );
        return;
      }
      ...
      setTxPending(txHash);
      setInvoiceSuccess(
        "⏳ Lệnh chuyển tiền thành công! Đang đợi thợ đào xác thực khối trên Sepolia...",
      );
      ...
      for (let i = 0; i < 40; i++) {
        ...
        try {
          ...
          console.error("Đang đọc tiến trình receipt ngầm...", err);
        }
      }
      ...
      try {
        ...
        if (response.data.success) {
          setInvoiceSuccess(
            "🎉 Thanh toán viện phí thành công! Trạng thái hệ thống đã được đồng bộ đồng nhất.",
          );
          setTxPending("");
          await loadInvoices();
        } else {
          setInvoiceError(
            `Cảnh báo: Tiền đã trừ trên Blockchain, nhưng máy chủ backend trả về lỗi: ${response.data.message}`,
          );
        }
      } catch (apiErr) {
        console.error("Lỗi gọi API hậu thanh toán:", apiErr);
        setInvoiceError(
          "⚠️ Tiền đã chuyển thành công qua Smart Contract, nhưng xảy ra lỗi kết nối đồng bộ cơ sở dữ liệu.",
        );
      }
    } catch (err) {
  ...
  const handleApproveRequest = async (request) => {
    ...
    if (!window.ethereum) {
      setError(
        "Vui lòng tải ví điện tử MetaMask để thực hiện ký xác thực danh tính bảo mật!",
      );
      return;
    }
    ...
    try {
      ...
      if (walletAddress.toLowerCase() !== request.patientWallet.toLowerCase()) {
        setError(
          `Sai địa chỉ ví! Vui lòng chọn tài khoản ví kết thúc bằng: ...${request.patientWallet.slice(-6)}`,
        );
        setApprovingId(null);
        return;
      }
      ...
      showSuccess(
        "Ký số danh tính thành công! Đang truyền gói tin bảo mật tới hệ thống tổng đài...",
      );

      const res = await axios.post(
        `${BASE_URL}/access/requests/${request._id}/approve`,
        { signature },
        { headers },
      );
      ...
      if (res.data.success) {
        showSuccess(
          `🎉 Duyệt quyền thành công! Giao dịch băm bảo mật đã ghi nhận lên Blockchain.`,
        );
        await loadAccessRequests();
      } else {
        setError(
          res.data.message || "Cơ sở dữ liệu từ chối xử lý chữ ký số này.",
        );
      }
    } catch (err) {
      if (err.code === 4001)
        setError(
          "Thao tác hủy bỏ: Bạn đã từ chối ký số thông điệp trên MetaMask.",
        );
  ...
  const handleRevokeAccess = async (request) => {
    if (
      !window.confirm(
        `Bạn có chắc muốn thu hồi quyền truy cập của BS. ${request.doctorName}?`,
      )
    )
      return;
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(
          `✅ Đã thu hồi quyền truy cập của BS. ${request.doctorName} thành công!`,
        );
        await loadAccessRequests();
      } else {
      ...
      setError(
        "Lỗi thu hồi quyền: " + (err.response?.data?.message || err.message),
      );
    } finally {
      setApprovingId(null);
    }
  ...
  const handleSaveBasic = async () => {
    ...
    try {
      ...
      setError("Lỗi đường truyền kết nối máy chủ!");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBookAppointment = async (e) => {
    e.preventDefault();
    if (!formAppointment.hospitalName) {
      setError("Vui lòng chỉ định Bệnh viện bạn muốn đến khám!");
      return;
    }
    if (!formAppointment.specialty) {
      setError("Vui lòng chọn Chuyên khoa đăng ký khám!");
      return;
    }
    if (!formAppointment.date) {
      setError("Vui lòng chọn thời gian ngày hẹn khám bệnh!");
      return;
    }
    if (!formAppointment.caKham) {
      setError("Vui lòng chỉ định Ca khám thích hợp (Ca Sáng / Ca Chiều)!");
      return;
    }
    ...
    try {
      ...
      if (dataRecord.success) {
        const resHistory = await fetch(
          `${BASE_URL}/visits/my?patientId=${userId}`,
          { headers },
        );
  ...
  const Field = ({ label, value }) => (
    ...
    >
      ...
      <div style={{ fontWeight: 600, color: PRIMARY, marginTop: 2 }}>
        {value || "—"}
      </div>
    </div>
  );
          ...
          {[
            {
              ...
              value: loadingHistory ? "..." : completedList.length,
            },
            {
              id: "choKham",
              icon: "⏳",
              label: "Lịch hẹn đang chờ",
              value: loadingHistory ? "..." : pendingList.length,
            },
            {
              id: "donThuoc",
              icon: "💊",
              label: "Toa thuốc đã cấp",
              value: loadingHistory
                ? "..."
                : completedList.filter((r) => r.huongDieuTri).length,
            },
          ].map((card) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  boxShadow: isSelected
                    ? `0 0 0 2px ${PRIMARY_MED}, 0 4px 20px rgba(26,79,168,0.15)`
              ...
              >
                ...
                <div style={{ fontSize: 13, color: GRAY_TEXT, marginTop: 4 }}>
                  {card.label}
                </div>
              </div>
            );
          ...
          <div style={{ padding: "24px" }}>
            ...
            {tab === "info" &&
              (loading ? (
                ...
                <>
                  <h4 style={{ color: PRIMARY, marginTop: 0 }}>
                  ...
                  >
                    ...
                    <Field
                      ...
                      value={patient?.dob?.substring(0, 10)}
                    />
                    <Field
                      label="Giới tính công khai"
                      value={
                        patient?.gender === "Male"
                          ? "Nam"
                    ...
                    />
                    <Field
                      label="Mã hóa địa chỉ ví Web3 liên kết"
                  ...
                  >
                    ...
                    <Field
                      label="Tiền sử dị ứng bản thân"
                      value={patient?.diUng}
                    />
                    <Field
                      label="Bệnh lý nền mắc phải"
                      value={patient?.tienSuBenh}
                    />
                    <Field
                      label="Triệu chứng lâm sàng ghi nhận"
                      value={patient?.trieuChungLamSang}
                    />
                  ...
                  >
                    ...
                    >
                      <h4 style={{ color: PRIMARY, margin: 0 }}>
                        {activeStatFilter === "luotKham" &&
                          "📋 Danh sách bệnh án đã hoàn tất khám"}
                        {activeStatFilter === "choKham" &&
                          "⏳ Danh sách lịch hẹn đang chờ tiếp đón"}
                    ...
                    {loadingHistory ? (
                      ...
                      >
                        {historyList
                          .filter((record) => {
                            if (activeStatFilter === "luotKham")
                              return record.status === "completed";
                            if (activeStatFilter === "choKham")
                              return record.status === "pending";
                            if (activeStatFilter === "dangKham")
                              return record.status === "examining";
                            if (activeStatFilter === "donThuoc")
                              return record.huongDieuTri;
                            return true;
                          })
                          .map((record, index, filteredArr) => (
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  Mã ca khám #{filteredArr.length - index}
                                </span>
                                <div
                                  style={{
                                    display: "flex",
                              ...
                              >
                                <div>
                                  <strong>Cơ sở y tế tiếp quản:</strong>{" "}
                                  {record.hospitalName ||
                                    "Hệ thống tổng hợp VNmedID"}
                                </div>
                                <div>
                                  <strong>Chuyên khoa đăng ký:</strong>{" "}
                                  {record.specialty || "Khám đa khoa tổng quát"}
                                </div>
                                {record.doctorName && (
                                  <div>
                                    <strong>
                                      Bác sĩ chuyên môn phụ trách:
                                    </strong>{" "}
                                    ThS. BS {record.doctorName}
                                  </div>
                                )}
                                <div>
                                  <strong>Triệu chứng khai báo trước:</strong>{" "}
                                  {record.trieuChungLamSang ||
                                    "Không khai báo."}
                                </div>
                                {record.status === "completed" && (
                                  <>
                                    <div style={{ marginTop: 4 }}>
                                      ...
                                      >
                                        {record.chanDoanChuyenMon ||
                                          "Đang đợi ký số kết luận"}
                                      </span>
                                    </div>
                                    <div
                                      style={{
                                    ...
                                    >
                                      <strong style={{ color: PRIMARY_MED }}>
                                        💊 Chỉ định toa thuốc & Hướng điều trị
                                        bổ sung:
                                      </strong>
                                      <br />
                                      {record.huongDieuTri ||
                                        "Không ghi nhận kê đơn."}
                                    </div>
                                  </>
                                )}
                              </div>
                  ...
                  >
                    ...
                    >
                      ...
                      gian thực trực tiếp từ mạng phi tập trung Sepolia Testnet
                    ...
                    {loadingBlockchain ? (
                      ...
                      >
                        🔄 Đang truy quét dữ liệu phân đoạn khối Smart
                        Contract...
                      </div>
                    ) : blockchainError ? (
                      <div
                      ...
                      >
                        ℹ️ Tài khoản định danh này chưa được ghi nhận dữ liệu
                        băm lịch sử trên mạng lưới Blockchain.
                      </div>
                    ) : (
                      <div
                        style={{
                      ...
                      >
                        ...
                        >
                          <div>
                            <strong>
                              Khóa tài khoản bệnh nhân (Patient Identity):
                            </strong>{" "}
                            <code
                              style={{
                                background: "#0047d4",
                            ...
                            >
                              {blockchainData.patientAddress}
                            </code>
                          </div>
                          <div>
                            <strong>
                              Địa chỉ ví Oracle xử lý (Hospital Master
                              Authority):
                            </strong>{" "}
                            <code
                              style={{
                            ...
                            >
                              {blockchainData.hospitalAddress}
                            </code>
                          </div>
                        </div>
            ...
            {tab === "register" && (
              <div style={{ maxWidth: 600 }}>
                <h4 style={{ color: PRIMARY, marginTop: 0, marginBottom: 20 }}>
                  Đặt lịch khám bệnh trực tuyến liên kết cơ sở
                </h4>
                <form onSubmit={handleBookAppointment}>
                  {/* 1. Chọn Bệnh Viện */}
                  <div style={{ marginBottom: 16 }}>
                    <label style={labelStyle}>
                      Cơ sở y tế mong muốn tiếp nhận{" "}
                    ...
                    >
                      <option value="">
                        -- Click chọn cơ sở bệnh viện tương thích --
                      </option>
                  ...
                  >
                    <label style={labelStyle}>
                      Thời gian ngày đặt lịch hẹn khám bệnh{" "}
                      <span style={{ color: "red" }}>*</span>
                    </label>
                    <DatePicker
                      style={{
                      ...
                      onChange={(val) =>
                        setFormAppointment((p) => ({ ...p, date: val }))
                      }
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <label style={labelStyle}>
                      Ca khám mong muốn <span style={{ color: "red" }}>*</span>
                    </label>
                    <select
                      style={inputStyle}
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <label style={labelStyle}>
                      Triệu chứng lâm sàng sơ bộ / Lý do khám
                    </label>
                    <textarea
                  ...
                  >
                    {saving
                      ? "Đang xử lý biểu mẫu dữ liệu..."
                      : "Hoàn tất gửi yêu cầu giữ chỗ hẹn khám"}
                  </button>
                </form>
              </div>
            )}
            ...
            {tab === "invoice" &&
              (loadingInvoice ? (
                ...
                >
                  ℹ️ Hiện tại không ghi nhận hóa đơn dịch vụ viện phí nào cần
                  thanh toán cho mã tài khoản này.
                </div>
              ) : (
                <div
                ...
                >
                  <h4 style={{ color: PRIMARY, margin: "0 0 4px 0" }}>
                    Bảng kê khai viện phí dịch vụ tổng kết
                  </h4>
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              ...
                              {invoice.description ||
                                "Chi phí khám bệnh & cấp phát vật tư y tế"}
                            </div>
                            ...
                            <div>
                              <strong>Số tiền quy đổi viện phí:</strong>{" "}
                              <span
                                style={{
                                  color: "#E24B4A",
                                  fontWeight: 700,
                            ...
                            <div>
                              ...
                              >
                                {Number(invoice.totalVND || 0).toLocaleString(
                                  "vi-VN",
                                )}
                                đ
                              </span>
                          ...
                          <div style={{ display: "flex", gap: 8 }}>
                            <button
                              onClick={() =>
                                setOpenInvoiceId(isOpen ? null : invoice._id)
                              }
                              style={{
                                background: WHITE,
                                color: PRIMARY,
                        ...
                        {isOpen && (
                          ...
                          >
                            ...
                            {invoice.items?.length > 0 ? (
                                ...
                                <tbody>
                                  {invoice.items.map((item, idx) => (
                                    ...
                                    >
                                      ...
                                      >
                                        {Number(
                                          item.unitPrice || item.priceVND || 0,
                              ...
                              >
                                Không có chi tiết thuốc trong hóa đơn này.
                              </div>
                            )}
                          </div>
                        )}
            ...
            {tab === "access" &&
              (loadingAccess ? (
                <div
                  style={{ textAlign: "center", padding: 20, color: GRAY_TEXT }}
                >
                  Đang tìm nạp danh sách phân quyền mật mã...
                </div>
                ...
                >
                  ℹ️ Chưa có yêu cầu xin cấp quyền truy cập dữ liệu bảo mật nào
                  từ phía Bác sĩ điều trị.
                </div>
              ) : (
                <div
                  style={{ display: "flex", flexDirection: "column", gap: 16 }}
                >
                  ...
                  {accessRequests.map((req) => (
                    ...
                    >
                      ...
                      >
                        <div>
                          <strong>Họ tên Bác sĩ yêu cầu:</strong> ThS. BS{" "}
                          {req.doctorName || "Ẩn danh"}
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          >
                            "
                            {req.reason ||
                              "Phục vụ công tác chẩn đoán bệnh án lâm sàng."}
                            "
                          </span>
                        </div>
                        <div>
                      ...
                      <div>
                        {req.status === "approved" ? (
                          ...
                          <button
                            ...
                            style={{
                              background:
                                approvingId === req._id ? GRAY_TEXT : PRIMARY,
                              color: WHITE,
                              border: "none",
                              padding: "10px 20px",
                              borderRadius: 8,
            ...
            {tab === "edit" && (
              <div style={{ maxWidth: 600 }}>
                ...
                >
                  ...
                  <div>
                    <label style={labelStyle}>
                      Ngày tháng năm sinh (Yêu cầu khớp CCCD)
                    </label>
                    <input
                      style={inputStyle}
                      type="date"
                      value={formBasic.dob}
                      onChange={(e) =>
                        setFormBasic((p) => ({ ...p, dob: e.target.value }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormBasic((p) => ({ ...p, gender: e.target.value }))
                      }
                    >
                  ...
                  <div>
                    <label style={labelStyle}>
                      Số điện thoại di động liên lạc
                    </label>
                    <input
                      style={inputStyle}
                      type="text"
                      ...
                      onChange={(e) =>
                        setFormBasic((p) => ({ ...p, phone: e.target.value }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      style={inputStyle}
                      type="text"
                      value={formBasic.address}
                      onChange={(e) =>
                        setFormBasic((p) => ({ ...p, address: e.target.value }))
                      }
                    />
                  </div>
                </div>
              ...
              <div style={{ maxWidth: 600 }}>
                ...
                >
                  <div>
                    ...
                    <select
                      style={inputStyle}
                      value={formHealth.nhomMau}
                      onChange={(e) =>
                        setFormHealth((p) => ({
                          ...p,
                          nhomMau: e.target.value,
                        }))
                      }
                    >
                  ...
                  <div>
                    <label style={labelStyle}>
                      Tiền sử dị ứng vật tư / Dược phẩm / Thực phẩm
                    </label>
                    <input
                      style={inputStyle}
                      type="text"
                      value={formHealth.diUng}
                      onChange={(e) =>
                        setFormHealth((p) => ({ ...p, diUng: e.target.value }))
                      }
                  ...
                  <div>
                    <label style={labelStyle}>
                      Bệnh lý nền mãn tính mắc phải
                    </label>
                    <input
                      style={inputStyle}
                      type="text"
                      value={formHealth.tienSuBenh}
                      onChange={(e) =>
                        setFormHealth((p) => ({
                          ...p,
                          tienSuBenh: e.target.value,
                        }))
                      }
                  ...
                  <div>
                    <label style={labelStyle}>
                      Triệu chứng lâm sàng ghi nhận tự thân gần đây
                    </label>
                    <input
                      style={inputStyle}
                      type="text"
                      value={formHealth.trieuChung}
                      onChange={(e) =>
                        setFormHealth((p) => ({
                          ...p,
                      ...
                      placeholder="Ví dụ: Thường xuyên đau đầu râm ran mệt mỏi..."
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      value={formHealth.ghiChu}
                      onChange={(e) =>
                        setFormHealth((p) => ({ ...p, ghiChu: e.target.value }))
                      }
                      placeholder="Các lưu ý đặc biệt khác cho bác sĩ khi kê đơn thuốc..."
                    />
                  </div>
                </div>
                <button
